...
fun processFunction(token: Token, tokens: ListIterator<Token>, context: ExpressionContext): Expression {
    ...
    if (function.size?.equals(params.size) == false) throw InvalidExpressionError(
        "fishcute.celestial.expressions.Function $name takes ${function.size} parameter${if (function.size == 1) "" else "s"}, but ${params.size} ${if(params.size==1) "was" else "were"} provided")
...
fun buildExpressionTree(input: ArrayList<Token>, context: ExpressionContext): ExpressionTree {
    ...
    if (builder == null) throw InvalidExpressionError("fishcute.celestial.expressions.Expression can not be empty")
    return builder.end()
}
